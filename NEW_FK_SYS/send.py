...
def html_to_pdf(html_string, pdf_location, landscape=None):
    ...
    open(pdf_location, 'wb').write(r.content)
...
def get_keys(receipt):
    with open(f"receipts/template_{receipt.replace(' ','_')}.html", "r", encoding='utf-8') as f:
...
def get_subject(receipt_name):
    with open("info.json", "r", encoding='utf-8') as f:
        all_info = json.load(f)
    print(all_info[receipt_name.replace(" email", "")]['subject'])
    return all_info[receipt_name.replace(" email", "")]['subject']

def get_template(receipt_name: str):
    with open(f"receipts/template_{receipt_name.replace(' email', '')}.html", "r", encoding='utf-8') as f:
        temp = f.read()
    return temp
...
def get_message_paper(order, login):
    ...
    if order['position'] == "landscape":
        html_to_pdf(temp, "paper.pdf", landscape=True)
    else:
        html_to_pdf(temp, "paper.pdf")
